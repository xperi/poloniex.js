module.exports = (function() {
    ...
    var crypto  = require('crypto'),
          ...
          nonceClient = require('nonce')(),
          Redlock = require('redlock');
      var redlock = new Redlock([redisClient],{
              retryCount:  10000,
              retryDelay:  500, // time in ms
      });
      var resource = 'locks:poloniex';
      var apiResource = 'locks:poloniex:api';
      var ttl = 2000;
    ...
    Poloniex.prototype = {
        ...
        _request: function(options, callback) {
            ...
            redlock.lock(apiResource, ttl).then(function(lock) {
              request(options, function(err, response, body) {
                ...
                setTimeout(function(){
                   return lock.unlock().catch(function(err) {
                   	console.error("redlock _request : " + err);
                   });
                },200)
        	    });
            });
        ...
        makeNonce(callback) {
          //console.log("Date :"+new Date().getTime() + " makeNonce");
          redlock.lock(resource, ttl).then(function(lock) {
            var nonceKey = "nonceKey"
            redisClient.get(nonceKey, function(error, lastTimeStamp){
             if(!error&&lastTimeStamp){
                var now = new Date()

                var timestamp = Math.pow(10, 2) * now;
                var offset = timestamp - lastTimeStamp;
                if (offset < 200) {
                   timestamp = lastTimeStamp + 200 - offset;
                }
                var result = timestamp.toString();
                redisClient.set(nonceKey, now.toString(), function(err){
                  if(err){
                    console.error("makeNonce set error: " + err);
                  }
                  callback(result);
                  lock.unlock().catch(function(err) {
                     console.error("redlock makeNonce : " + err);
                   });
                });
              } else {
                if(error){
                  console.error("makeNonce get error:" + error);
                }
                var now = new Date();
                var timestamp = Math.pow(10, 2) * now;
                var result = timestamp.toString();
                redisClient.set(nonceKey, result, function(err){
                  if(err){
                    console.error("makeNonce set error: " + err);
                  }
                  callback(result);
        ...
        _private: function(command, parameters, callback) {
          var getPrivateHeaders = this._getPrivateHeaders;
          var request = this._request;
           this.makeNonce(function(nonce){
              var options;
              if (typeof parameters === 'function') {
                  callback = parameters;
                  parameters = {};
