    ...
    Poloniex.prototype = {
        ...
        makeNonce(callback) {
          ...
          redlock.lock(resource, ttl).then(function(lock) {
            ...
            redisClient.get(nonceKey, function(error, lastDate){
             if(!error&&lastDate){
                var now = new Date()
                var offset = new Date() - lastDate;
                if (offset < 200) {
                   now = lastDate + 200 - offset;
                }
                var timestamp = Math.pow(10, 2) * now;
                var result = timestamp.toString();
                redisClient.set(nonceKey, now.toString(), function(err){
                  if(err){
                    console.error("makeNonce sel error: " + err);
                  }
                  callback(result);
                ...
                redisClient.set(nonceKey, now.toString(), function(err){
                  if(err){
                    console.error("makeNonce selt error: " + err);
                  }
                  callback(result);
                  lock.unlock().catch(function(err) {
                     console.error("redlock makeNonce : " + err);
